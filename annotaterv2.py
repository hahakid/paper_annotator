...
# @Last Modified by:   ouyangkid     Contact: ouyangkid@buaa.edu.cn
...
class Example(Frame):
    def __init__(self, parent):
        ...
        #self.controlCommand = {'q':"unTag", 'ctrl+z':'undo'}  # q & ctrl+z 按钮不可冲突
        self.controlCommand = {'ctrl+z':'undo'}  # q & ctrl+z 按钮不可冲突
        ...
        self.configFile = "configs/xia1.config" #"configs/default.config"
        self.entityRe = r'\<.*?\#.*?\*\/\>(?!\#)' # r'\[\@.*?\#.*?\*\](?!\#)'
        ...
        self.initUI()
        #self.LongLabelPairFlag=False # 争对需要跨段标注的标签配对控制
    ...
    def initUI(self):
        ...
        ubtn.grid(row=2, column=self.textColumn +1, pady=2)

        ubtn = Button(self, text="NewMap", command=self.savenewPressCommand)
        ubtn.grid(row=3, column=self.textColumn +1, pady=2)
        ...
        cbtn.grid(row=7, column=self.textColumn + 1, pady=2)

        self.cursorName = Label(self, text="Cursor: ", foreground="Blue", font=(self.textFontStyle, 12, "bold"))
        self.cursorName.grid(row=8, column=self.textColumn +1, pady=2)
        self.cursorIndex = Label(self, text=("row: %s\ncol: %s" % (0, 0)), foreground="red", font=(self.textFontStyle, 12, "bold"))
        self.cursorIndex.grid(row=9, column=self.textColumn + 1, pady=2)
    ...
    def executeCursorCommand(self,command):
        ...
        try:
            ...
            selected_string = self.text.selection_get()  # 选定字符串，可以加入后处理删除连续空格，以空格开头，以空格结尾
    ...
    def RemoveInvalidSpace(self, targetStr):
        targetStr = targetStr.strip()  # 首尾空格
        return ' '.join(targetStr.split())  # 中间连续空格

    def difReplace(self, replaceType):
        #numlist='0123456789'
        cont=self.pressCommand[replaceType]
        if replaceType in '0123456789':
            prefix="<" +cont+">"  # 前缀=<str>
            postfix="</" +cont+">" # 后缀=</str>
        elif replaceType in ['q', 'w', 'e']:
            prefix="<" +cont+">"  # 前缀=<str>
            postfix="" # 后缀=</str>
        elif replaceType in ['a', 's', 'd']:
            prefix=""  # 前缀=<str>
            postfix="<" +cont+">" # 后缀=</str>
        else:
            prefix=""
    ...
    def replaceString(self, content, string, replaceType, cursor_index):
        if replaceType in self.pressCommand:
            ...
            string=self.RemoveInvalidSpace(string)  # 清理空格

            prefix,postfix = self.difReplace(replaceType)
            #prefix="<" +self.pressCommand[replaceType]+">"  # 前缀=<str>
            #postfix="</" +self.pressCommand[replaceType]+">"  # 后缀=</str>

            new_string = prefix+ string + postfix
    ...
    def writeFile(self, fileName, content, newcursor_index):
        ...
        if len(fileName) > 0:
            if "-mark" in fileName:
                ...
                new_name = fileName.replace('.txt','-mark.txt')
                ann_file = open(new_name, 'w')
                ann_file.write(content)
                ann_file.close()
    ...
    def setMapShow(self):
        ...
        mapLabel.grid(row=0, column = self.textColumn+2, columnspan=2, rowspan=1, padx=5)
        ...
        self.configListLabel.grid(row=row + 1, column=self.textColumn + 2, columnspan=2, rowspan=1, padx=5)
        self.configListBox = Combobox(self, values=getConfigList(), state='readonly')
        self.configListBox.grid(row=row + 2, column=self.textColumn + 2, columnspan=2, rowspan=1, padx=5)
